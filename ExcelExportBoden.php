...
$sDbTabellenName = "Bodenmesswerte";
$sDbName  = "MeteoVaduz";
...
header("Content-Disposition: attachment; filename=BodenMesswerteauszug.$sDateiTypEndung"); 
...
if($_GET['sURLSensor']=="*"){
	$dbResultat = $db_handler->db_query("select Bezeichnung,Masseinheit from Sensoren where Identifikation in (7,8,9,10)");
}else{
...
if($_GET['sURLSensor']=="*"){
	...
	$dbResultat = $db_handler->db_query("select Messzeit,W0,W1,W2,W3 from $sDbTabellenName where Messzeit between '".$_GET['sDatumVon']."' and '".$_GET['sDatumBis']."' order by Messzeit");
